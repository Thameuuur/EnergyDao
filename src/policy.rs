...
//use near_sdk_sim::lazy_static_include::syn::Member; 
...
impl RoleKind {
    ...
    pub fn match_user(&self, user: &UserInfo) -> bool {
        match self {
            RoleKind::Member(stakers) => stakers.contains(&user.account_id),
    ...
    /// Returns the number of stakers
    pub fn get_stakers_size(&self) -> Option<usize> {
        match self {
            RoleKind::Group(stakers) => Some(stakers.len()),
            _ => None,
        }
    }
    
    pub fn add_staker(&mut self, account_id: &AccountId) -> Result<(), ()> {
        match self {
            RoleKind::Member(stakers) => {
                stakers.insert(account_id.clone());
                Ok(())
            }
            _ => Err(()),
        }
    ...
    pub fn remove_staker(&mut self, account: &AccountId) -> Result<(), ()> {
        match self {
            RoleKind::Member(stakers) => {
...
pub fn default_policy(council: Vec<AccountId>, stakers: Vec<AccountId>) -> Policy {
    Policy {
        roles: vec![
            RolePermission {
                name: "stakers".to_string(),
                kind: RoleKind::Member(stakers.into_iter().collect()),
                permissions: vec!["Suggestion:*".to_string(),
                                  "Vote:*".to_string(),
                                  ].into_iter().collect(),
...
impl VersionedPolicy {
    ...
    pub fn upgrade(self) -> Self {
        match self {
            VersionedPolicy::Default{council, stakers} => {
                VersionedPolicy::Current(default_policy(council,stakers))
            }
...
impl Policy {
    ...
    pub fn add_member_to_role(&mut self, role: &String, member_id: &AccountId) {
        for i in 0..self.roles.len() {
            if (&self.roles[i].name == role) && (&role.to_string() == "council") {
                self.roles[i]
                    .kind
                ...
                if (&self.roles[i].name == role) && (&role.to_string() == "stakers") {
                    self.roles[i]
                        .kind
                        .add_staker(member_id)
                        .unwrap_or_else(|()| {
                            env::log_str(&format!("ERR_ROLE_WRONG_KIND:{}", role));
                        });
                    return;
    ...
    pub fn proposal_status(
        ...
        if proposal.submission_time.0 + self.proposal_period.0 < env::block_timestamp() {
         ...
         for role in roles {
            ...
            println!("vote_counts: {:?}", vote_counts[Vote::Approve as usize]);
...
mod tests {
    ...
    fn test_add_role() {
        ...
        let stakers = vec![accounts(0), accounts(1)];
        let mut policy = default_policy(council,stakers);
